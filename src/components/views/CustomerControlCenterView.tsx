...
export const CustomerControlCenterView: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'STAGES' | 'FINANCES' | 'DOCS' | 'PHOTOS' | 'SERVICE'>('STAGES');
    ...
    <div className="space-y-6 animate-in fade-in duration-150">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-7 border border-slate-200/80 shadow-2xs space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-200/80">
          ...
          <div>
            <span className="text-[11px] font-bold text-slate-400 uppercase">Project Manager</span>
            <p className="text-xs font-semibold text-slate-800 mt-0.5 truncate">Amit Sharma</p>
          </div>
        </div>
      ...
      <div className="flex border-b border-slate-200 bg-white rounded-t-2xl px-4 pt-2 shadow-2xs overflow-x-auto">
        ...
        >
          ...
          <span>15-Stage Workflow Timeline</span>
          <span className="text-[10px] px-1.5 py-0.2 rounded-full bg-slate-100 text-slate-600">
            {currentProject.stages.filter(s => s.status === 'COMPLETED').length}/15 Done
          </span>
        </button>
        ...
        <div className="bg-white rounded-2xl p-5 sm:p-6 border border-slate-200/80 shadow-2xs space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 pt-2">
            {allDocuments.map(doc => (
              <div key={doc.id} className="p-4 rounded-xl border border-slate-200 hover:border-amber-300 bg-slate-50/50 hover:bg-white transition-all group flex flex-col justify-between">
                <div>
      ...
      {activeTab === 'PHOTOS' && (
        <div className="bg-white rounded-2xl p-5 sm:p-6 border border-slate-200/80 shadow-2xs space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 pt-2">
            {allPhotos.map(p => (
              <div key={p.id} className="rounded-2xl border border-slate-200 overflow-hidden bg-white shadow-2xs group flex flex-col justify-between">
